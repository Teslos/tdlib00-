...
/*
Changes to hooke by Evgenii Rudnyi:

  1) converted to class
  2) all the memory is allocated within the class
  3) all the settings are made within the class 
...
double
Hooke::best_nearby(double *delta, double *point, double prevbest, int nvars)
{
     double    minf, ftmp;
     int       i;
     minf = prevbest;
     ...
     for (i = 0; i < nvars; i++)
       point[i] = z[i];
// EBR     
// bug in gcc?  -O3 breaks the code
// if I put here cout << "minf " << minf << endl;
// everything works fine
// without -O3 everything works fine
     return minf;
}

int
Hooke::hooke(int nvars, double *startpt, double *endpt, double rho, 
